...
API_KEY = "<>"
EMBEDDING_URL = "http://localhost:8000/v2/models/ensemble_model/infer"
...
async def get_embeddings(session: aiohttp.ClientSession, input_text: List[str], input_type: str) -> float:
    ...
    # Prepare inputs in Triton format
    inputs = [{
        "name": "text", 
    ...
    payload = {
        "inputs": inputs,
        "outputs": [{"name": "embeddings"}]  # Adjust the output name as required by your model
    }
